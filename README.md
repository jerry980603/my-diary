...
存在**你開這個網站的那台裝置、那個瀏覽器的儲存空間**裡（IndexedDB），
不是相簿、不是「檔案」App、也不在 GitHub 上。

- 從相簿選照片上傳時是**複製一份**進去，原本相簿的照片完全不受影響
- 手機上傳的留在手機，電腦上傳的留在電腦，**兩邊不會互相同步**
- 左側「我的紀錄」會顯示這台裝置用掉多少空間、還剩多少
- 同步過來但檔案不在這台的附件，會標示「☁️ 檔案在「手機」上，這台沒有」

### 怎麼避免附件不見

| 做法 | 說明 |
|---|---|
| **加到主畫面** | 最重要。iOS Safari 對「一般網頁」的資料有清除機制，安裝成主畫面 App 後穩定很多 |
| **持久化儲存** | 網站會自動向瀏覽器申請保護。左側顯示「🔒 資料已受保護」就是成功了 |
| **定期備份** | 按「💾 備份全部」時選「確定」＝連附件一起打包成 JSON，存到雲端硬碟 |
| **把附件搬到另一台** | 在有檔案的那台匯出「含附件」的備份，到另一台按「📥 匯入備份檔」 |

> 顯示「⚠️ 未受保護」不代表資料馬上會不見，只是瀏覽器沒給保證。備份還是最可靠的。
